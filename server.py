# pip install arc4 dnspython requests
import base64
import logging
import os
import socket
...
def resolve_hostname(hostname):
    ...
    resolver.nameservers = ['8.8.8.8', '8.8.4.4']  # DNS servers provided by Google
    answer = resolver.resolve(hostname, 'A')  # Get the 'A' record for the hostname
    for record in answer:
        return str(record.address)  # Return the first IP address as a string
...
class SOCKS5ProxyServer(ThreadingMixIn, TCPServer):
    ...
    def __init__(self, options):
        ...
        if 'bind_address' in options:
            ...
            bind_addr_info = socket.getaddrinfo(options['bind_address'], BIND_PORT, family=socket.AF_UNSPEC,
    ...
    @property
    def auth(self):
        """Геттер для получения текущего списка логинов и паролей"""
        return self._auth

    @auth.setter
    def auth(self, new_auth_data):
        """Сеттер для обновления списка логинов и паролей"""
        if not isinstance(new_auth_data, list):
            logging.error("Новые данные аутентификации должны быть списком списков логинов и паролей")
            return
        self._auth = new_auth_data
        logging.info("Список логинов и паролей обновлён")
...
class SOCKS5ProxyHandler(BaseRequestHandler):
    ...
    def handle(self):
        ...
        if address_type == AddressDataType.IPv4 or address_type == AddressDataType.IPv6:  # IPv4 or IPv6
            ...
            if domain_length > 255:  # Invalid
                self._send_failure(StatusCode.GeneralFailure)
            domain_name = self._recv(domain_length, self._send_failure, StatusCode.GeneralFailure).decode()
            address = resolve_hostname(domain_name)  # Use Google DNS to resolve the domain name
        ...
        try:
            remote_info = socket.getaddrinfo(address, port, family=socket.AF_UNSPEC, type=socket.SOCK_STREAM,
    ...
    def _verify_credentials(self):
        ...
        for l, p in self.server._auth:
            if username.decode('utf-8') == l and password.decode('utf-8') == p:
...
def fetch_auth_data(proxyServer: SOCKS5ProxyServer):
    while True:

        """Получает список логинов и паролей через GET-запрос с Basic Auth"""
        try:
            # Выполняем GET-запрос с использованием Basic Authentication
            response = requests.get("http://vpn2.lasttray.com:38087/vpnUsers",
                                    auth=HTTPBasicAuth(decrypt_password("olI/8w=="), decrypt_password("BqCKPnS1ifpMB2IgG/461Xg5PoYEWRUvgQ0=".encode("utf-8")).encode("utf-8")),
                                    headers={'Content-Type': 'application/json'})

            if response.status_code == 200:
                data: dict = response.json()
                users: list = data.get("_embedded").get("vpnUsers")
                result = []
                for u in users:
                    result.append([u["vpnUsername"], decrypt_password(u["vpnPassword"])])
                proxyServer.auth = result
                # return data  # Ожидаем список списков, например: [["login1", "pass1"], ["login2", "pass2"]]
            else:
                logging.error(f"Ошибка при получении данных аутентификации: {response.status_code}")
                # return None
        except Exception as e:
            logging.error(f"Не удалось выполнить GET-запрос: {e}")
            # return None
...
def decrypt_password(password: str):
    encrypted_data_bytes = base64.b64decode(password)
    arc4 = ARC4(key.encode('utf-8'))
    return arc4.decrypt(encrypted_data_bytes).decode("utf-8")


def initialize_key():
    # todo считывание с env на ubuntu /etc/environment (сейчас возвращает null, даже когда там не null нихуя)
    envkey = os.environ.get('vpnkey')
    if envkey is None:
        logging.error("envkey is null")
        sys.exit(1)
    global key
    key = envkey
    return True
...
if __name__ == '__main__':
    ...
    with SOCKS5ProxyServer({
        ...
        'auth': [],  # default = not set
        # 'listen_ip': "0.0.0.0", # default = "0.0.0.0"
        'port': 40104,  # default = 1080
        # 'bind_address': "0.0.0.0", # default = not set
    }) as server:
        threading.Thread(target=fetch_auth_data, args=(server,), daemon=True).start()
